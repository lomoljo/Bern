...
public class JavaCommon {
  ...
  /**
   * Use this method rather than {@link RuleContext#getLabel()} when generating a label meant to be
   * consumed by BUILD file tooling such as buildozer.
   *
   * @param ruleContext the rule context
   * @return the label of the current rule if it hasn't been generated by a macro, or a label for
   *    the macro (valid on the level of parsed, but unevaluated BUILD files) if it has.
   */
  public static Label getLabelForTooling(RuleContext ruleContext) {
    String generatorName = ruleContext.attributes().get("generator_name", Type.STRING);
    if (generatorName != null && !ruleContext.getLabel().getName().equals(generatorName)) {
      try {
        return Label.create(ruleContext.getLabel().getPackageIdentifier(), generatorName);
      } catch (LabelSyntaxException unused) {
        // Tooling label creation is best-effort, fall back to the rule label.
      }
    }
  ...
  public JavaTargetAttributes.Builder initCommon(
    ...
    javaTargetAttributes.setTargetLabel(getLabelForTooling(ruleContext));
